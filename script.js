...
//Отобразить всех котов из cats
function renderCats(cats) {
    //Получение контейнера для котов
    var container = document.getElementById('cats-container');
    
    //Для каждого объекта в cats
    for (let i = 0; i < cats.length; i++) {
        
        //Создать div для текущего кота
        var catBlock = document.createElement('div');
        
        //Задать элементу класс cat-block
        catBlock.classList.add('cat-block');
        
        //Получить текущий объект из cats
        var cat = cats[i];

        //Преобразовать в избранном false в Нет и True в да
        var isFavourite;
        ...
        //Записать внутрь элемента div код для отображения кота
        catBlock.innerHTML = `
            <h2>${cat.name}</h2>
            <img src="${cat.img_link}">
            <p>Возраст: ${cat.age} лет</p>
            <p>Рейтинг: ${cat.rate}</p>
            <p>В избранном: ${isFavourite}</p>
            <p>Описание: ${cat.description}</p>
        `;

        //Добавить элемент с котом в контейнер для котов
        container.appendChild(catBlock);
    }
}


//Вызов функции для отображения котов из cats
renderCats(cats);